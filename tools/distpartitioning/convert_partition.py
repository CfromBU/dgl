...
def _create_edge_data(
    edgeid_offset, etype_ids, num_edges
):
    eid = (
        th.arange(
            edgeid_offset,
    ...
    inner_edge = (
        th.ones(num_edges, dtype=RESERVED_FIELD_DTYPE["inner_edge"])
    )
    return eid, etype, inner_edge


def _create_node_data(
    ntype, uniq_ids, reshuffle_nodes, inner_nodes
):
